...
# --------------------------------------- FAILURES ------------------------------------------------
    ...
    # ----------------------------------- BALANCING HISTORY TABLE -----------------------------------
    # BALANCING HISTORY OPERATIONS:
    #   [initializeTransferHistory, storeTransfer, getTransfer, deleteTransfer]

    # Cleanup local variables for global namespace

    # INITIALIZER: initializeTransferHistory
    # INPUT: N/A
    # OUTPUT: N/A
    ...
    def getError(uuid):
        connect, cursor = ArbitrageDatabase.connect()
        error = ArbitrageDatabase.getError(cursor, uuid)
        disconnect(connect)
        print("getError", error)
        return error

    # FUNCTION: getErrors
    def getErrors(period, uuids=None):
        connect, cursor = ArbitrageDatabase.connect()
        errors = ArbitrageDatabase.getErrors(cursor, period, uuids)
        disconnect(connect)
        print("getErrors", period, uuids, error)
        return errors

    # FUNCTION: deleteError : Currently not in use
    def deleteError():
        # FINE FOR NOW
        pass

    # ------------------------------------- MISCELLANEOUS ------------------------------------------
    # Figure out where the these functions belong

    # Only works for arbitrage database
    # This is just a wrapper however, probably requires some work
    def retrieveAll(table_name):
        connect, cursor = ArbitrageDatabase.connect()
        values = ArbitrageDatabase.retrieveAll(cursor, table_name)
        disconnect(connect)
        return values

    #   Returns a list of currencies available for a given exchange
    def getCurrencies(exchange):
        connect, cursor = ArbitrageDatabase.connect() 
        curr_list = ArbitrageDatabase.getCurrencies(cursor,exchange)
        disconnect(connect)
        return curr_list
    ...
    def mutualCurrencies(exchange_list):
        ...
        for ex in exchange_list:
            if i == 1:
                ex_currs = ArbitrageDatabase.getCurrencies(cursor,exchange)
